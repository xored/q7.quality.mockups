...
/**
 * The GCombo class was copied from the CCombo class. It represents a selectable
 * user interface object that combines a text field and a list and issues
 * notification when an item is selected from the list.
 * <p>
 * GCombo was written to work around certain limitations in the native combo
 * box. Specifically, on win32, the height of a GCombo can be set; attempts to
 * set the height of a Combo are ignored. GCombo can be used anywhere that
 ...
 *      information</a>
 */
public class VeryCustomCombo extends Composite {
	...
	int incrementalSearchValue = 1500; // 1.5 sec default

	StringBuffer incrementalSearchText = new StringBuffer();

	long currentTime = 0;

	// PSD - check prefix
	static final String PACKAGE_PREFIX = "com.xored.q7.quality.mockups.issues.parts.internal."; //$NON-NLS-1$
	 ...
	 * @see Widget#getStyle()
	 */
	public VeryCustomCombo(Composite parent, int style) {
		...
		listener = new Listener() {
			...
			public void handleEvent(Event event) {
				...
				if (VeryCustomCombo.this == event.widget) {
					comboEvent(event);
					return;
				}
		...
		filter = new Listener() {
			...
			public void handleEvent(Event event) {
				...
				if (shell == VeryCustomCombo.this.getShell()) {
					handleFocus(SWT.FocusOut);
				}
			}
		};
		...
		int[] textEvents = { SWT.DefaultSelection, SWT.DragDetect, SWT.KeyDown, SWT.KeyUp, SWT.MenuDetect,
	...
	void initAccessible() {
		AccessibleAdapter accessibleAdapter = new AccessibleAdapter() {
			 ...
			 * '&' character in the given string. If there are no '&' characters
	...
	void listEvent(Event event) {
		...
		case SWT.FocusIn: {
			...
			if (incrementalSearch)
				incrementalSearchText = new StringBuffer();
			break;
		}
		case SWT.MouseUp: {
			if (event.button != 1)
		...
		case SWT.KeyDown: {
			...
			event.doit = e.doit;
			if (incrementalSearch
					& (event.keyCode >= 97 && event.keyCode <= 122 || event.keyCode >= 48 && event.keyCode <= 57)) {
				applyIncrementSearch(event);
				event.doit = false;
			}
			break;
	...
	void textEvent(final Event event) {
		...
		case SWT.KeyDown: {
			...
			// Check for characters and integers only for incremental change
	...
	private void applyIncrementSearch(final Event event) {
		if (event.keyCode >= 97 && event.keyCode <= 122 || event.keyCode >= 48 && event.keyCode <= 57) {
			if ((getStyle() & SWT.READ_ONLY) != 0) {
				incrementalSearchText.append(event.character);
				int oldIndex = getSelectionIndex();
				int selectIndex = findItem(incrementalSearchText.toString());
				if (oldIndex != selectIndex && selectIndex >= 0) {
					select(selectIndex);
					Event e = new Event();
					e.time = event.time;
					e.stateMask = event.stateMask;
				...
				Runnable timer = new Runnable() {
					...
					@Override
					public void run() {
						if (System.currentTimeMillis() - currentTime > incrementalSearchValue)
							incrementalSearchText = new StringBuffer();

					}
				};
				event.display.timerExec(incrementalSearchValue, timer);

			}
		}
	}

	private int findItem(String string) {
		// Assumes list is already sorted
		String[] items = list.getItems();
		for (int i = 0; i < items.length; i++) {
			if (items[i].startsWith("<")) // Ignore default value for starting
											// with open bracket
				continue;
			int compare = items[i].compareToIgnoreCase(string);
	...
	public int getIncrementalSearchValue() {
		return incrementalSearchValue;
	}

	public void setIncrementalSearchValue(int incrementalSearchValue) {
		this.incrementalSearchValue = incrementalSearchValue;
	}
}
